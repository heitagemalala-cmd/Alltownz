// backend/server.js - ZERO DEPENDENCIES
const http = require('http');
const fs = require('fs');
const path = require('path');

// In-memory database (this will be replaced by JSON file storage if you ask)
const DB = {
    users: [
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && url === '/api/auth/login') {
        return parseBody(req, (data) => {
            ...
            if (user) {
                ...
                sendJSON(res, { token: 'demo-token', user: { ...user, password: undefined, shop: shop || null } });
            } else {
    ...
    if (req.method === 'POST' && url === '/api/shops/register') {
        return parseBody(req, (data) => {
            ...
            const newShop = { id: DB.shops.length + 1, owner_id: data.owner_id, name: data.name, description: data.description, approved: false };
    ...
    if (req.method === 'PUT' && url.startsWith('/api/admin/approve-shop/')) {
        ...
        if (shop) { shop.approved = true; return sendJSON(res, { message: 'Shop approved successfully!' }); }
    ...
    if (req.method === 'PUT' && url.startsWith('/api/admin/toggle-user/')) {
        ...
        if (user) { user.verified = !user.verified; return sendJSON(res, { message: `User ${user.verified ? 'Enabled' : 'Disabled'}`, user }); }
    ...
    if (req.method === 'POST' && url === '/api/orders') {
        return parseBody(req, (data) => {
            const order = { ...data, id: Date.now(), order_id: 'ALL-' + Date.now().toString(36).toUpperCase(), created_at: new Date().toISOString(), status: 'pending' };
            DB.orders.push(order);
            sendJSON(res, { order_id: order.order_id, id: order.id, status: 'pending', message: 'Order placed successfully!' });
        });
    }

    // SERVE STATIC FILES
    if (req.url === '/' || req.url === '/index.html') return serveStatic(res, path.join(__dirname, '../index.html'), 'text/html');
    if (req.url.endsWith('.html')) return serveStatic(res, path.join(__dirname, '..', req.url), 'text/html');
    if (req.url.endsWith('.css')) return serveStatic(res, path.join(__dirname, '..', req.url), 'text/css');
    if (req.url.endsWith('.js')) return serveStatic(res, path.join(__dirname, '..', req.url), 'application/javascript');
    
    sendJSON(res, { error: 'Not found' }, 404);
});

const PORT = 3000;
server.listen(PORT, '0.0.0.0', () => console.log(`✅ AllTownz Marketplace running on port ${PORT}`));
